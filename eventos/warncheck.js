...
client.on("guildMemberAdd", async (member) => {
    ...
    let roles = await check_roles(userid)
...
// 
    ...
    if (warns["points"] >= 3){ 
        member.roles.add([config.roles.adv3, config.roles.adv2, config.roles.adv1])
        return
    };
    if (warns["points"] >= 2){
        member.roles.add([config.roles.adv2, config.roles.adv1])
        return
    };
    if (warns["points"] >= 1){
