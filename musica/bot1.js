...
client1.login(config.musica.token.bot1);
const index = require(`../index`);

const DisTube1 = require('distube')
const distube = new DisTube1(client1, { searchSongs: true, emitNewSongOnly: true });
client1.distube = distube;
let inici = true;
...
client1.on("ready",()=>{
client1.voice.connections.forEach(m=>
    m.disconnect()
    )
})
...
client1.distube
    ...
    client1.on("message",async message=>{
      ...
      async  function verificadb(){
        if(index.db2.db.exists(`/bot1`)){
           let obj= await index.db2.db.getData(`/bot1`)
           index.db2.db.delete(`/bot1`)
          
           const canal = client1.channels.cache.get(obj.channel.id)
           let test = await canal.messages.fetch({ limit: 5 }).catch(console.error)
            
           test.forEach(element => {
            if(element.id==obj.id){
                element.content = [element.content.slice(0, 1), "1", element.content.slice(1)].join('');
                message = element
                if(message){
                    executa(message)
                }
        ...
        if(!message.author.bot && message.guild){
            if(message.member.voice.channel){
                if(message.content.startsWith(config.prefixo+"1")){
