...
client3.login(config.musica.token.bot3);
const index = require(`../index`);

const DisTube3 = require('distube')
const distube = new DisTube3(client3, { searchSongs: true, emitNewSongOnly: true });
client3.distube = distube;
...
client3.on("ready",()=>{
    client3.voice.connections.forEach(m=>
        m.disconnect()
        )
    })
...
client3.distube
    ...
    client3.on("message", message=>{
        ...
        async  function verificadb(){
          if(index.db2.db.exists(`/bot3`)){
             let obj= await index.db2.db.getData(`/bot3`)
             index.db2.db.delete(`/bot3`)
            
             const canal = client3.channels.cache.get(obj.channel.id)
             let test = await canal.messages.fetch({ limit: 5 }).catch(console.error)
              
             test.forEach(element => {
              if(element.id==obj.id){
                  element.content = [element.content.slice(0, 1), "3", element.content.slice(1)].join('');
                  message = element
                  if(message){
                      executa(message)
                  }
        ...
        if(!message.author.bot && message.guild){
            if(message.member.voice.channel){
                if(message.content.startsWith(config.prefixo+"3")){
