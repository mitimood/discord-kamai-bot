...
client4.login(config.musica.token.bot4);
const index = require(`../index`);

const DisTube4 = require('distube')
const distube = new DisTube4(client4, { searchSongs: true, emitNewSongOnly: true });
client4.distube = distube;
...
client4.on("ready",()=>{
    client4.voice.connections.forEach(m=>
        m.disconnect()
        )
    })
...
client4.distube
    ...
    client4.on("message", message=>{
        ...
        async  function verificadb(){
          if(index.db2.db.exists(`/bot4`)){
             let obj= await index.db2.db.getData(`/bot4`)
             index.db2.db.delete(`/bot4`)
            
             const canal = client4.channels.cache.get(obj.channel.id)
             let test = await canal.messages.fetch({ limit: 5 }).catch(console.error)
              
             test.forEach(element => {
              if(element.id==obj.id){
                  element.content = [element.content.slice(0, 1), "4", element.content.slice(1)].join('');
                  message = element
                  if(message){
                      executa(message)
                  }
        ...
        function executa(message){if(!message.author.bot && message.guild){
            if(message.member.voice.channel){
                
                if(message.content.startsWith(config.prefixo+"4")){
