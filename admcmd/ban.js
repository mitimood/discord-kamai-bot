            ...
            for(let i=0;i<(await result).members.length || i < (await result).users.length; i++){    
                ...
                if((await result).members[i]!=undefined){
                    try{
                        ...
                        await (await result).members[i].user.send(`Aplicado por(${msg.author.tag}-----${msg.author.id})\n\nVocê foi banido de KAMAITACHI, por: `+reason+ `\nCaso queira recorrer ao seu ban, entre no servidor ${invite.url}`)
                    }catch{

                    }finally{
                        let prune_days = 0
                        if(reason == "Scam") prune_days = 1
                        await Banning((await result).members[i].user.id, `[${msg.author.id}] `+reason, msg.guild, prune_days)}
                    }
